...
namespace Microsoft.Azure.SqlDatabase.ElasticScaleNetCore.Query {
  ...
  public sealed class MultiShardDataReader : DbDataReader, IDataReader, IDisposable, IDataRecord {
    ...
    private ReadOnlyCollection<DbColumn> _schemaComparisonTemplate;
    private ReadOnlyCollection<DbColumn> _finalSchemaTable;
    ...
    /// <summary>
    /// Returns a <see cref="ReadOnlyCollection<DbColumn>"/> that describes the column metadata of the MultiShardDataReader.
    /// </summary>
    /// <param name="avoidExtension">set a value to indicate this method and not the extension method, value is ignored internally.</param>
    /// <returns>A <see cref="ReadOnlyCollection<DbColumn>"/> that describes the column metadata.</returns>

    public ReadOnlyCollection<DbColumn> GetColumnSchema(bool avoidExtension) {
      return GetPropertyOrVariableWithStateCheck<ReadOnlyCollection<DbColumn>>(_finalSchemaTable);
    }
    ...
    private void ValidateReaderSchema(LabeledDbDataReader labeledReader) {
      ...
      ReadOnlyCollection<DbColumn> currentDataTable = reader.GetColumnSchema();
    ...
    /// </remarks>
    private static void ValidateSchemaTableStructure(ShardLocation shardLocation, ReadOnlyCollection<DbColumn> toValidate, ReadOnlyCollection<DbColumn> expected) {

      // this is irrelavant now
    ...
    /// </exception>
    private static void ValidateSchemaTableContents(ShardLocation shardLocation, ReadOnlyCollection<DbColumn> toValidate, ReadOnlyCollection<DbColumn> expected) {

      for(int curIndex = 0; curIndex < expected.Count; curIndex++) {
        DbColumn colToValidate = toValidate[curIndex];
        DbColumn colTemplate = expected[curIndex];
  ...
  public class pseudoDbColumn : DbColumn {
